...
const delete_user_by_username = async (req, res) => {
    try {
        ...
        if (!username) {
            return res.status(400).json({ message: "Invalid Request: Username is required", ok: false });
        }
        ...
        return res.status(200).json({ message: `User ${username} deleted successfully`, ok: true });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: "Server Error", ok: false });
    }
